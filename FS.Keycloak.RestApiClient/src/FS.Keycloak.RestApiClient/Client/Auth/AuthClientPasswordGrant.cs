...
public class AuthClientPasswordGrant : HttpClient
{
    ...
    public async Task<KeycloakApiToken> GetToken(CancellationToken cancellationToken)
    {
        ...
        var token = JsonContent.DeserializeObject<KeycloakApiToken>(tokenJson, _jsonSerializerSettings);
        return token;
    }
}
